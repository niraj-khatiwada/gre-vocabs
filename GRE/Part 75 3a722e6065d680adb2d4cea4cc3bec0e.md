# Part 75

- **ostracize**
    
    *verb*
    
    to exclude, reject, or deliberately keep someone away from a group or society.
    
    syn: **shun; secluded; banish; alienate; insular**
    
    🧠 ostracize = imagine ostrich are placed separately in a zoo because they attack people = isolated
    
- **insularize**
    
    *verb*
    
    to isolate from a group
    
    syn: **ostracize**
    
- **intrepid**
    
    DO NOT GET CONFUSED WITH insipid
    
    *adjective*
    
    fearless; brave; courageous; bold
    
    🧠 trepid = derived from word **trepidation** which literally means a feeling of fear, anxiety or worry
    
    🧠 intrepid = opposite of trepid(trepid means timid, fearful, apprehensive) = fearless
    
- **plucky**
    
    *adjective*
    
    syn: **intrepid; brave**
    
    🧠 plucky = pluck the bravery
    
- **mettlesome**
    
    *adjective*
    
    brave; full of energy
    
    syn: **intrepid; plucky; hardy** (yes like Tom Hardy)
    
    🧠 mettlesome = metal + some = hard & brave
    
- **foment**
    
    *verb*
    
    to stir up, encourage, or provoke something—especially something negative like trouble, conflict, rebellion, or unrest.
    
    syn: **incite; agitate; inflame; provoke**
    
    🧠 foment = fume+moment = inflame = incite
    
- **content**
    
    *adjective*
    
    feeling satisfied and happy
    
    🧠 content = complete
    
- **hegemony**
    
    *noun*
    
    dominance or control over certain area or something
    
    syn: **dominance; authority; supremacy; control**
    
    🧠 hegemony = derived from **hegemon** which means a dominant ruler
    
    **The empire maintained hegemony over the region for centuries.**
    
    **The country gained cultural hegemony through its movies, language, and media.**
    
- **prolific**
    
    *adjective*
    
    DO NOT GET CONFUSED WITH profound
    
    large in number or quantity
    
    syn: **profuse**, **copious**
    
    **“prolific music composer”**
    
    **“prolific number of fruits”**
    
    **“Tigers are prolific breeders.”**
    
- **accolade**
    
    *noun*
    
    an award or praise granted as a special honor
    
    🧠 accolade = sounds similar to applaud = honor/award
    
    **The scientist received many accolades for her groundbreaking research.**
    
    **The film earned critical accolades worldwide.**
    
    ![image.png](Part%2075/image.png)
    
- **propitious**
    
    *adjective*
    
    favorable, advantageous, or likely to lead to success.
    
    syn: **auspicious; opportune**
    
    🧠 propitious = proper + auspicious
    
- **auspicious**
    
    *adjective*
    
    syn: **propitious**
    
- **digress**
    
    *verb*
    
    deviate away from main subject
    
    syn: **stray; wander; drift**
    
    🧠 digress = diverge+great = deviate from main topic
    
    🧠 **di**gression = divert from gre session = you keep getting way from gre study(main topic)
    
- **inveterate**
    
    *adjective*
    
    firmly established through long practice; deeply ingrained; unlikely to change
    
    syn: **ingrained; entrenched; hardened**
    
    🧠 inveterate = veteran = established and hardened belief
    
    **inveterate gambler**
    
- **winsome**
    
    *adjective*
    
    charming, attractive, and pleasing in a simple, innocent way.
    
    syn: **charming; congenial; delightful; amiable**
    
    🧠 winsome = win some hearts = charming
    
- **revelry**
    
    *noun*
    
    to take great pleasure or delight in something ⇒ usually in a party and celebration
    
    🧠 revelry = derived from word revel = boisterous party or celebration
    
    ---
    
    verb
    
    revel
    
    ![image.png](Part%2075/image%201.png)
    
- **merrymaking**
    
    *adjective*
    
    syn: **revelry**
    
    🧠 merrymaking = marry + making = boisterous celebration
    
    🧠 merrymaking = merry(happy & cheerful)+making = celebration